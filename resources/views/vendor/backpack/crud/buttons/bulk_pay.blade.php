@if ($crud->hasAccess('bulkDelete') && $crud->get('list.bulkActions'))
    <a href="javascript:void(0)" onclick="bulkPayEntries(this)" class="btn btn-sm btn-secondary bulk-button"><i class="la la-trash"></i> {{ trans('backpack::crud.pay') }}</a>
@endif

@push('after_scripts')
    <script>
        if (typeof bulkPayEntries != 'function') {
            function bulkPayEntries(button) {
